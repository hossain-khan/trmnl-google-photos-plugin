...
async function checkServiceStatus() {
    ...
    try {
        ...
        if (response.ok) {
            ...
            badge.setAttribute('data-tooltip', `API healthy • ${responseTime}ms • v${data.version || '0.2.0'} • Click to refresh`);
...
async function fetchPhotoData() {
    ...
    if (!albumUrl) {
        ...
        errorContainer.innerHTML = '<div class="error-message"><strong>⚠️ Validation Error:</strong> Please enter a Google Photos album URL</div>';
        return;
    }
    ...
    if (!albumUrl.includes('photos.app.goo.gl') && !albumUrl.includes('photos.google.com')) {
        ...
        errorContainer.innerHTML = '<div class="error-message"><strong>⚠️ Invalid URL:</strong> Please enter a valid Google Photos shared album URL (photos.app.goo.gl/... or photos.google.com/share/...)</div>';
        return;
    }
    ...
    jsonContent.innerHTML = '<div class="spinner"></div><p style="text-align: center; color: #666; margin-top: 10px;">Fetching photo data from API...</p>';
    ...
    try {
        ...
        const totalTime = serverTime !== 'N/A' ? `${serverTime} (server) + ${clientLatency}ms (network)` : `${clientLatency}ms (total)`;
        ...
        if (!response.ok) {
            ...
            switch (response.status) {
                case 400:
                    ...
                    break;
                case 403:
                    errorMsg = 'Album access denied. The album may be private or link sharing may be disabled. Please ensure the album has link sharing enabled.';
                    break;
                case 404:
                ...
                case 502:
                case 503:
                    errorMsg = 'Server error. The service may be temporarily unavailable. Please try again later.';
                    break;
                default:
        ...
        if (data.photo_url) {
            ...
            if (data.metadata && data.metadata.original_width && data.metadata.original_height) {
                metadataItems.push(`<div class="metadata-item"><span class="metadata-label">📐 Dimensions:</span><span class="metadata-value">${data.metadata.original_width} × ${data.metadata.original_height}px</span></div>`);
            }
            if (data.aspect_ratio) {
                metadataItems.push(`<div class="metadata-item"><span class="metadata-label">📏 Aspect Ratio:</span><span class="metadata-value">${data.aspect_ratio}</span></div>`);
            }
            if (data.megapixels) {
                metadataItems.push(`<div class="metadata-item"><span class="metadata-label">🎨 Megapixels:</span><span class="metadata-value">${data.megapixels} MP</span></div>`);
            }
            ...
            imagePreviewContent.innerHTML = '<div style="color: #999; text-align: center;">📷<br>No photo available</div>';
        }
        ...
        if (error.message.includes('Album access denied') || error.message.includes('403')) {
            troubleshootingTips = '<br><br><strong>💡 Troubleshooting:</strong><br>1. Open the album in Google Photos<br>2. Click the share button (⋯)<br>3. Enable "Share album" or "Create link"<br>4. Copy the new link and try again';
        } else if (error.message.includes('Album not found') || error.message.includes('404')) {
            troubleshootingTips = '<br><br><strong>💡 Troubleshooting:</strong><br>• Verify the URL is correct<br>• Check if the album still exists<br>• Try accessing the album in your browser first';
        } else if (error.message.includes('Invalid album URL')) {
            troubleshootingTips = '<br><br><strong>💡 Expected format:</strong><br>• photos.app.goo.gl/XXXXXXXXX<br>• photos.google.com/share/XXXXXXXXX';
        }
